"""Hybrid scripted Adroit hammer controller.

This script uses a deterministic pickup, then records the first hammer strike as
the reference rhythm. The following strikes use bounded arm/wrist corrections to
return to a consistent retract pose, then execute the proven impact stroke
without changing it so the hammer-head geom contacts the nail geom each swing.
"""
...
# Keep the pickup/lift that works well, then use one recorded nail-driving cycle
# as the rhythmic strike template. The hand/finger commands are kept from this
# template. During closed-loop strikes, only the first four arm/wrist action
# channels are corrected during the retract/setup phase. The impact phase is
# intentionally left on the template because small corrections there can miss the
# nail geom while still looking close by task-site distance.
RECORDED_ACTIONS = SCRIPTED_ACTIONS
PICKUP_ACTIONS = RECORDED_ACTIONS[:50]
STRIKE_TEMPLATE_ACTIONS = RECORDED_ACTIONS[50:70]
NUM_STRIKES = 3
SCRIPTED_ACTIONS = np.vstack(
    [PICKUP_ACTIONS, *[STRIKE_TEMPLATE_ACTIONS for _ in range(NUM_STRIKES)]]
)
STRIKE_START_STEP = len(PICKUP_ACTIONS)
...
def build_arm_delta_grid() -> np.ndarray:
    """Candidate bounded corrections for ARRx, ARRy, WRJ1, and WRJ0."""
    candidates = []
...
def phase_name(step: int) -> str:
    ...
    if step < STRIKE_START_STEP:
        return "pickup_and_lift"

    strike_number = (step - STRIKE_START_STEP) // STRIKE_CYCLE_STEPS + 1
    cycle_step = (step - STRIKE_START_STEP) % STRIKE_CYCLE_STEPS
    if cycle_step < 10:
        return f"strike_{strike_number}_retract"
    return f"strike_{strike_number}_impact"
...
def get_hammer_nail_geom_ids(env: gym.Env) -> tuple[int, tuple[int, ...]]:
    model = env.unwrapped.model
    hammer_head_geom_id = None
    nail_body_id = None

    for geom_id in range(model.ngeom):
        if model.geom(geom_id).name == HAMMER_HEAD_GEOM_NAME:
            hammer_head_geom_id = geom_id
            break

    for body_id in range(model.nbody):
        if model.body(body_id).name == NAIL_BODY_NAME:
            nail_body_id = body_id
            break

    if hammer_head_geom_id is None or nail_body_id is None:
        raise RuntimeError("could not find hammer head and nail geoms")
    ...
    return hammer_head_geom_id, nail_geom_ids


def hammer_nail_contact(
    env: gym.Env,
    hammer_head_geom_id: int,
    nail_geom_ids: tuple[int, ...],
) -> tuple[bool, float | None]:
    data = env.unwrapped.data
    closest_contact = None
    for contact_index in range(data.ncon):
        contact = data.contact[contact_index]
        contact_geoms = {int(contact.geom1), int(contact.geom2)}
        if hammer_head_geom_id in contact_geoms and any(
            nail_geom_id in contact_geoms for nail_geom_id in nail_geom_ids
        ):
            closest_contact = (
                float(contact.dist)
                if closest_contact is None
                else min(closest_contact, float(contact.dist))
            )
    return closest_contact is not None, closest_contact
...
def save_action_trajectory(
    ...
    np.savez_compressed(
        ...
        lookahead_steps=np.array(lookahead_steps, dtype=np.int64),
        num_strikes=np.array(NUM_STRIKES, dtype=np.int64),
        strike_start_step=np.array(STRIKE_START_STEP, dtype=np.int64),
...
def run_episode(
    ...
    env = gym.make(ENV_ID, render_mode=render_mode, max_episode_steps=200)
    ...
    action_count = len(replay_actions) if replay_actions is not None else len(SCRIPTED_ACTIONS)

    for step in range(action_count):
        if replay_actions is not None:
            action = replay_actions[step]
        elif step < STRIKE_START_STEP:
            action = PICKUP_ACTIONS[step]
        else:
            strike_index = (step - STRIKE_START_STEP) // STRIKE_CYCLE_STEPS
            cycle_step = (step - STRIKE_START_STEP) % STRIKE_CYCLE_STEPS
            base_action = STRIKE_TEMPLATE_ACTIONS[cycle_step]
            if (
                closed_loop
                and strike_index > 0
                and cycle_step < IMPACT_PHASE_STEP
                and len(strike_reference_offsets) == STRIKE_CYCLE_STEPS
            ):
                action = select_closed_loop_strike_action(
                    env,
                    base_action,
                    strike_reference_offsets,
                    cycle_step,
                    lookahead_steps,
                )
            else:
        ...
        tool_nail_dist = np.linalg.norm(tool - nail)
        head_nail_contact, _ = hammer_nail_contact(env, hammer_head_geom_id, nail_geom_ids)
        if step >= STRIKE_START_STEP:
            strike_index = (step - STRIKE_START_STEP) // STRIKE_CYCLE_STEPS
            cycle_step = (step - STRIKE_START_STEP) % STRIKE_CYCLE_STEPS
            if strike_index < NUM_STRIKES:
                strike_min_distances[strike_index] = min(
                    strike_min_distances[strike_index], tool_nail_dist
                )
                if head_nail_contact:
                    strike_head_nail_contacts[strike_index] = True
                    strike_contact_steps[strike_index].append(step)
                if closed_loop and replay_actions is None and strike_index == 0:
                    strike_reference_offsets.append(tool - nail)

        should_log = (
            verbose
            or step % 10 == 0
            or step == first_success_step
            or step == action_count - 1
        )
        if should_log:
            print(
                f"{step:03d} {phase_name(step):18s} "
    ...
    if save_actions_path is not None:
        save_action_trajectory(
            ...
            seed=seed,
            closed_loop=closed_loop and replay_actions is None,
