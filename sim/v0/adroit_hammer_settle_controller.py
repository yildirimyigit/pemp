...
RETRACT_ACTIONS = RECORDED_ACTIONS[50:60].copy()
IMPACT_ACTIONS = RECORDED_ACTIONS[60:70].copy()
    ...
    for arm_delta in ARM_DELTA_CANDIDATES:
        ...
        for horizon_step in range(horizon):
            if horizon_step == 0:
                ...
                rollout_action = IMPACT_ACTIONS[template_step - 10] if template_step >= 10 else RETRACT_ACTIONS[template_step]
...
def run_episode(
    ...
    env = gym.make(ENV_ID, render_mode=render_mode, max_episode_steps=500)
