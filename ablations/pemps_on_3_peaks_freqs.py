...
for i in range(20):
    ...
    pemp_2 = PEMP(input_dim=dpe+dg, output_dim=dy, n_max=n_max, m_max=m_max, encoder_hidden_dims=enc_dims, decoder_hidden_dims=dec_dims, batch_size=batch_size, device=device)
    optimizer2 = torch.optim.Adam(lr=3e-4, params=pemp_2.parameters())

    pemp_3 = PEMP(input_dim=dpe+dg, output_dim=dy, n_max=n_max, m_max=m_max, encoder_hidden_dims=enc_dims, decoder_hidden_dims=dec_dims, batch_size=batch_size, device=device)
    ...
    freqs, _ = torch.sort(torch.rand(4)*1000)
    print(freqs)

    pe = torch.zeros((num_models, t_steps, dpe))  # keeping pe's for each model with different frequencies
    pe[0]=generate_positional_encoding(d_model=dpe, frequency_scaler=freqs[0]) / dpe  # /dpe since dpe-many pes and only one SM
    pe[1]=generate_positional_encoding(d_model=dpe, frequency_scaler=freqs[1]) / dpe
    pe[2]=generate_positional_encoding(d_model=dpe, frequency_scaler=freqs[2]) / dpe
    pe[3]=generate_positional_encoding(d_model=dpe, frequency_scaler=freqs[3]) / dpe

    # %%
    obs = torch.zeros((num_models, batch_size, n_max, dpe+dy), dtype=torch.float32, device=device)
    tar_x = torch.zeros((num_models, batch_size, m_max, dpe), dtype=torch.float32, device=device)
    ...
    def prepare_masked_batch(t: list, traj_ids: list):
        ...
        for model_ind in range(num_models):
            for i, traj_id in enumerate(traj_ids):
                ...
                obs[model_ind, i, :n, :dpe] = pe[model_ind, n_ids] # PE(t)
                obs[model_ind, i, :n, dpe:] = traj[n_ids]  # SM(t)
                obs_mask[i, :n] = True
                
                tar_x[model_ind, i, :m, :dpe] = pe[model_ind, m_ids]
                tar_y[i, :m] = traj[m_ids]
                tar_mask[i, :m] = True

    val_obs = torch.zeros((num_models, batch_size, n_max, dpe+dy), dtype=torch.float32, device=device)
    val_tar_x = torch.zeros((num_models, batch_size, t_steps, dpe), dtype=torch.float32, device=device)
    ...
    def prepare_masked_val_batch(t: list, traj_ids: list, fixed_ind=None):
        ...
        for model_ind in range(num_models):
            for i, traj_id in enumerate(traj_ids):
                ...
                val_obs[model_ind, i, :n, :dpe] = pe[model_ind, n_ids]
                val_obs[model_ind, i, :n, dpe:] = traj[n_ids]
                val_obs_mask[i, :n] = True

                val_tar_x[model_ind, i, :, :dpe] = pe[model_ind]
    ...
    for i in range(num_models):
        freqs_code += str(freqs[i].item())[:4] + '_'
    freqs_code = freqs_code[:-1]

    timestamp = int(time.time())
    root_folder = f'../outputs/ablation/{num_peaks}_peak/{pose_code}/{arch_code}/{freqs_code}/{str(timestamp)}/'
    ...
    torch.save(freqs, f'{root_folder}freqs.pt')
