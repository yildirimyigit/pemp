...
class CMPE(nn.Module):  # CMPE: Cyclic Movement Primitives with Positional Encoding
    def __init__(self, input_dim=1, output_dim=1, n_max=10, m_max=10, window_length=10, encoder_hidden_dims=[128,128,128], decoder_hidden_dims=[128,128,128], batch_size=32, device='cpu'):
        ...
        self.lstm = nn.LSTM(input_dim + output_dim, encoder_hidden_dims[0], batch_first=True)
    ...
    def forward(self, obs, tar, obs_mask, latent=False):
        # obs: (batch_size, n_max, window_length, input_dim+output_dim)
        # tar: (batch_size, m_max, input_dim)
        # obs_mask: (batch_size, n_max)

        lstm_in = obs.view(-1, self.window_length, self.input_dim+self.output_dim)
        lstm_out, _ = self.lstm(lstm_in)  # (batch_size*n_max, window_length, encoder_hidden_dims[0](=lstm_hidden_dim))

        lstm_out_last = lstm_out[:, -1, :]  # Take the last hidden state (batch_size*n_max, lstm_hidden_dim)
        
        # reshape back
        lstm_out_last_reshaped = lstm_out_last.view(self.batch_size, self.n_max, -1)  # (batch_size, n_max, lstm_hidden_dim)

        # encoding
        encoded_obs = self.encoder(lstm_out_last_reshaped)  # Apply encoder to lstm output
    ...
    def val(self, obs, tar, obs_mask, latent=False):
        ...
        with torch.no_grad():
            ...
            lstm_out_last_reshaped = lstm_out_last.view(self.batch_size, self.n_max, -1)

            encoded_obs = self.encoder(lstm_out_last_reshaped)
            obs_mask_exp = obs_mask.unsqueeze(-1).type_as(encoded_obs)  # (batch_size, n_max, 1)
            masked_encoded_obs = encoded_obs * obs_mask_exp  # (batch_size, n_max, encoder_hidden_dims[-1])
