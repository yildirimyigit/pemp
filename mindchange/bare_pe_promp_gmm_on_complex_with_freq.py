...
for iteration in range(20):
    ...
    for idx, (promp, gmm) in enumerate(zip(promps, gmm_list)):
        ...
        # save promp+gmm models for later evaluation
        model_folder = f'{root_folder}saved_models/promp_{idx}/'
        if not os.path.exists(model_folder):
            os.makedirs(model_folder)
        # promp doesn't have a built-in save function
        with open(f'{model_folder}promp.pkl', 'wb') as f:
            import pickle
            pickle.dump(promp, f)
